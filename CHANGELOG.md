...
- Upgrade edugo-shared auth and gin middleware
...
- Simplify menu permission handling
- Allow school-level unit access
- fix: add 'write' action to writeActions for access mode computation
- feat: add access_mode (view/edit) to menu response for readonly support
...
- fix: add fallback from global to school-scoped context in RefreshToken
- fix: optimize token refresh TTL and reduce DB queries
...
- Bump EduGo infra and shared dependencies
- Recompute unit permissions only for members
...
- fix: address dev-to-main code review feedback
- fix: add IsActive: true to mock units in TestGetSchoolUnits_Success
- Add school units endpoint to API docs
- Bump Go module dependencies
- fix: validate unit membership in SwitchContext and filter active units
- fix: address Copilot code review feedback
- docs: update swagger docs for school units endpoint
- feat: enrich context selection with academic units
...
- Bump dependencies
- fix: eliminar stub FindBySchool del mock de MembershipRepository
- Add FindBySchool to mockMembershipRepo
- Bump internal dependency versions
- fix: remove is_active filter from FindByID methods to allow editing inactive records
- fix: add IsActive field to UpdateRoleRequest for role activation/deactivation
...
- ci: standardize pipelines - Node 24, retry sync, action upgrades
- pipeline
...
- chore: go mod tidy — add testify dependency for auth service tests
- test: add unit tests for auth service login and switch-context
- fix: address PR review — bounded concurrency, sync audit, log errors
- update
- perf: parallelize login and context-building DB queries
...
- fix: return copy from template cache and add nil check in CreateInstance
- Bump edugo module dependencies
- fix: apply audit findings — screen 404, query optimization, auth refactor
- fix: use sharedrepo.ErrNotFound instead of gorm.ErrRecordNotFound in auth service
- update
